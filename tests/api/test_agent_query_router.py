...
async def _fake_agent_service():
    return _FakeAgentService()


async def test_agent_query_requires_authentication():
...
async def test_agent_query_returns_answer_and_sources():
    app.dependency_overrides[get_current_user] = _test_user
    app.dependency_overrides[get_agent_query_service] = _fake_agent_service
    try:
        async with AsyncClient(
            transport=ASGITransport(app=app),
            base_url="http://test",
...
async def test_agent_query_without_conversation_id():
    app.dependency_overrides[get_current_user] = _test_user
    app.dependency_overrides[get_agent_query_service] = _fake_agent_service
    try:
        async with AsyncClient(
